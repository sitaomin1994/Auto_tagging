...
class SimpleGRU(nn.Module):
    def __init__(self, args):
        ...
        self.gru = nn.GRU(self.embedding_dim, self.n_hidden, self.num_layers, bidirectional = True)
        self.out = nn.Linear(self.n_hidden*3*2, self.n_out)
    ...
    def init_hidden(self, batch_size):
        return Variable(torch.zeros((self.num_layers*2, batch_size, self.n_hidden),device=torch.device("cuda")))